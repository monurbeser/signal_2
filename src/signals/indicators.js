...
function calcRSI(candles, period = 14) {
  const results = ti.RSI.calculate({ values: closes(candles), period });
...
function calcBBPosition(candles, period = 20, stdDev = 2) {
  ...
  const results = ti.BollingerBands.calculate({ values, period, stdDev });
...
function calcEMA(candles, period = 200) {
  const results = ti.EMA.calculate({
    values: closes(candles),
    period,
  });
